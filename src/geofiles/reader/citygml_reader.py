...
class CityGmlReader(GmlReader, XmlReader, BaseReader, ABC):
    ...
    def read_xml(self, xml: ET.Element) -> GeoObjectFile:
        ...
        cityObjects = xml.findall(".//cityObjectMember/Building")

        for cityObject in cityObjects:
            self.curr_object = GeoObject()
            self.curr_object.set_type("Building")
            id = cityObject.attrib.get("id")
            if id is None:
                id = str(uuid.uuid4())
            self.curr_object.meta_information["id"] = id
            for surfaceClass in ["GenericCityObject", "CeilingSurface", "InteriorWallSurface", "FloorSurface", "RoofSurface", "WallSurface", "GroundSurface", "ClosureSurface", "BuildingInstallation"]:
                self._internal_read_xml(result, cityObject, f".//{surfaceClass}")
